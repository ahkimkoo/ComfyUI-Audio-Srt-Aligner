"""
Alignment engine for generating SRT subtitles from audio and reference text.

Ported from audio-srt-aligner/align_to_srt.py. Contains all algorithm logic
for token-level alignment, waveform snapping, and SRT generation.

This module is designed to be used as a library by ComfyUI nodes or other
integrations — no CLI code is included.
"""
...
def create_alignment_config(
    ...
    beam_size: int = 5,
    start_lag: float = 0.03,
    end_hold: float = 0.12,
    min_gap: float = 0.03,
    snap_window: float = 0.30,
    no_waveform_snap: bool = False,
    max_unit_duration: float = 5.80,
    split_pause_gap: float = 0.55,
    max_split_depth: int = 2,
    max_early_lead: float = 0.04,
    ...
    Parameter ranges are clamped to safe minimums/maximums, mirroring the
    original CLI defaults and validation logic.
    """
    return AlignmentConfig(
        ...
        language=language,
        beam_size=beam_size,
        start_lag=max(0.0, start_lag),
        end_hold=max(0.0, end_hold),
        min_gap=max(0.0, min_gap),
        snap_window=max(0.05, snap_window),
        no_waveform_snap=bool(no_waveform_snap),
        max_unit_duration=max(2.0, max_unit_duration),
        split_pause_gap=max(0.15, split_pause_gap),
        max_split_depth=max(0, max_split_depth),
        max_early_lead=max(0.0, max_early_lead),
...
def run_alignment_pipeline(
    ...
    if not ref_tokens:
        raise ValueError("No valid tokens found in transcript text.")
...
def generate_srt_string(
    ...
    """Run alignment and return SRT content as a string instead of writing to a file.

    This is the primary integration point for ComfyUI nodes that need the SRT
    content in-memory rather than on disk.

    Args:
        audio_path: Path to the input audio file.
        reference_text: The transcript/reference text to align.
        config: Alignment configuration.
        progress: Optional callback for progress updates.

    Returns:
        Tuple of (srt_string, detected_language, srt_entries_count, coverage).

    Raises:
        ValueError: If audio file is missing, text is empty, or alignment fails.
    ...
    # Write reference text to a temporary file for the pipeline
    tmp_text = tempfile.NamedTemporaryFile(
        mode="w", suffix=".txt", prefix="align_ref_", encoding="utf-8", delete=False
    )
    tmp_text.write(reference_text)
    tmp_text.close()

    # Create a temporary output path
    tmp_output = tempfile.NamedTemporaryFile(
        mode="w", suffix=".srt", prefix="align_out_", delete=False
    )
    tmp_output.close()
    ...
    try:
        ...
        srt_string = Path(tmp_output.name).read_text(encoding="utf-8")
        return (
            srt_string,
            result.detected_language,
            result.srt_entry_count,
            result.coverage,
        )
    finally:
        # Clean up temporary files
        for tmp_path in (tmp_text.name, tmp_output.name):
            try:
                os.unlink(tmp_path)
            except OSError:
                pass
